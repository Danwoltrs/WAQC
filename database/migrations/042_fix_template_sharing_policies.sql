-- Migration 042: Fix Template Sharing Policies
-- Date: 2025-10-15
-- Purpose: Drop all existing policies and recreate them correctly with assigned_laboratories support

-- ========================================
-- ADD ASSIGNED LABORATORIES COLUMN (IF NOT EXISTS)
-- ========================================
...
DROP POLICY IF EXISTS "Global templates visible to all" ON quality_templates;
DROP POLICY IF EXISTS "Lab-assigned templates visible to lab members" ON quality_templates;
DROP POLICY IF EXISTS "Private templates visible to creator" ON quality_templates;
DROP POLICY IF EXISTS "Global admins can view all templates" ON quality_templates;
DROP POLICY IF EXISTS "Users can create lab templates" ON quality_templates;
DROP POLICY IF EXISTS "Users can update own templates or admins all" ON quality_templates;
DROP POLICY IF EXISTS "Users can delete own templates or admins all" ON quality_templates;

-- ========================================
-- CREATE NEW RLS POLICIES
-- ========================================

-- Policy 1: Global admins can see all templates (highest priority)
...
-- Policy 2: Global templates visible to all authenticated users
CREATE POLICY "Global templates visible to all" ON quality_templates
    FOR SELECT USING (
        is_global = true
    );

-- Policy 3: Lab-assigned templates visible to users in those labs
CREATE POLICY "Lab-assigned templates visible to lab members" ON quality_templates
    FOR SELECT USING (
        is_global = false
        AND assigned_laboratories IS NOT NULL
        AND assigned_laboratories != '{}'
        AND EXISTS (
            SELECT 1 FROM profiles
            WHERE id = auth.uid()
              AND laboratory_id = ANY(quality_templates.assigned_laboratories)
        )
    );

-- Policy 4: Private templates visible only to creator
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Migration 042 verification failed: assigned_laboratories column not created';
    END IF;

    RAISE NOTICE 'Migration 042 completed successfully: Fixed template sharing policies';
END;
$$;

SELECT 'Migration 042: Fix template sharing policies completed' as status;
