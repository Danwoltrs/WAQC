-- Migration 113: Force recreate client-to-supply-chain sync trigger
-- Description: Aggressively drop and recreate to clear any cached function definitions
-- Date: 2025-11-26
...
CREATE FUNCTION sync_client_to_supply_chain_entities()
RETURNS TRIGGER AS $$
DECLARE
    v_entity_id UUID;
BEGIN
...
'Automatically creates or links supply chain entity records (exporters, importers, roasters) when a client is created or updated based on their client_types. Fixed version without ON CONFLICT errors.';
...
SELECT 'Migration 113: Force recreated client-to-supply-chain sync trigger with CASCADE' as status;
