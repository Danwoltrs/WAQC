-- Wolthers Coffee QC System - Storage Enhancements Migration
...
-- Policy: Global admins can see all shelves
...
CREATE POLICY lab_shelves_client_view ON lab_shelves
  ...
  USING (
    ...
    AND EXISTS (
      ...
      WHERE p.id = auth.uid()
        AND c.id = lab_shelves.client_id
    )
  );

-- ========================================
-- 10. UPDATE TRIGGER FOR AUTOMATIC POSITION GENERATION
-- ========================================

CREATE OR REPLACE FUNCTION trigger_generate_positions_on_shelf_insert()
RETURNS TRIGGER AS $$
BEGIN
  -- Auto-generate positions when a new shelf is created
  PERFORM generate_storage_positions_for_shelf(NEW.id);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Note: We don't automatically create this trigger to allow manual control
-- Lab admins can call generate_storage_positions_for_shelf() explicitly
COMMENT ON FUNCTION trigger_generate_positions_on_shelf_insert IS 'Trigger function to auto-generate positions when shelf is created';

-- ========================================
-- 11. DATA MIGRATION FOR EXISTING POSITIONS
-- ========================================
