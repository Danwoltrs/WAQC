...
| `Thumbstick.tsx` | Well + knob (56 px); deadzone 14%; reports the vector plus grab/release; drag-the-well to relocate; springs to the nearer side on release; side in `localStorage['waqc.wheel.stickSide']`; idle fade to 35% after 2.5 s; knob tinted with the highlighted wedge. **Since 2026-09-09 it drives a cursor, not a pan** — see `stick-nav.ts` | — |
| `stick-nav.ts` | Pure: `moveCursor` (a scene point advanced at `cursorSpeed(m)` = 150 units/s × the thumb's linear deflection, held inside the rim so it slides round the circle and crosses the hub), `wedgeUnder` (the wedge under the cursor, any ring; the hub is nothing; nowhere keeps the previous), `followPoint`/`followCamera` (scroll only when the point leaves the inner 22 % margin) | `camera`, `hit-test` |
...
| Thumbstick | **A cursor (Daniel 2026-09-09, second phone test: "if I hold it down left, it should keep going down left, the further my thumb is from the center, the faster it goes, going back center slows down").** While the knob is held a cursor dot glides over the wheel in the stick's direction at a speed linear in its deflection; the wedge under it wears the highlight — the same focus ring the arrow keys move — and every boundary crossed is one haptic tick ("tac tac tac"). It slides round the rim and crosses the hub; the camera follows only when it would leave the inner box. Letting the knob go selects what is under the cursor: a wedge of an unframed family frames that FAMILY (the wedge stays highlighted so the next release picks it); a wedge inside the framed family toggles its pick. A tap on the glass while the knob is held selects the same way. A hold that never moved selects nothing. History: the first fix that day was a D-pad (one discrete step per push) — dead on a ring of nine, where one step is all there is — and before that the stick was a continuous pan that never worked at rest because the loop's first frame integrated with dt = 0 and stopped (fixed; the desktop edge pan had the same bug). |

Haptics via `src/lib/haptics.ts`: `navigator.vibrate` where it exists (Android Chrome);
iOS Safari has no vibrate API, so it toggles a hidden `<input type="checkbox" switch>`
(the iOS ≥ 17.4 switch haptic), which iOS honours only inside a trusted touch event —
ticks raised from the rAF loop are queued and played on the next touch event the page
sees. 8 ms on a pick, 12 ms double pulse at the cap, 5 ms on crossing a wedge or ring. The thumbstick is optional: a small
