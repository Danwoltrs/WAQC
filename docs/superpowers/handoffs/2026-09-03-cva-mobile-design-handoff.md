# Handoff — CVA mobile UI design canvas (updated 2026-09-09 evening: second phone test → stick is a cursor, tray collapses, iOS ticks queued)

**Resume point (updated 2026-09-09 evening):** Two phone-test rounds today, both fixed and **pushed to production** (SHAs at the bottom). The stick is now a **cursor** — hold it and a dot glides over the wheel in that direction, faster the further out, tick per wedge, release selects (the D-pad of the afternoon was one step and dead). A wheel tap collapses the descriptors tray. iOS haptic ticks are queued for the next touch event. **What is NOT verified is the iPhone haptic itself**: Daniel reported "no haptic feedback yet" after round one; round two may or may not fix it — the diagnostic is whether a plain tap on a 1–9 block buzzes (that path runs inside a trusted pointerdown). If it does not, the switch-input trick is not working on his phone/settings and no plumbing helps; if it does but the stick stays silent, iOS is not counting touchmove as a gesture and the loop's ticks should be moved onto pointerdown/up only. Next: Daniel's third phone pass; then the checklist tap-to-check decision and the SCA-102 §7.2 warning.
...
## 2026-09-09 evening — second phone test (pushed)

Daniel tested the afternoon build and sent three notes: (1) *"the joystick is still not fully functional, if I hold it down left, it should keep going down left, the further my thumb is from the center, the faster it goes, going back center slows down, like a circular motion"*; (2) *"the middle screen keeps showing there, if user taps the wheel, it should hide or go nearly full transparent"* (screenshot: the expanded descriptors tray over the wheel, "centre · zoom out" and "Hide stick" painted across it); (3) *"no haptic feedback yet either"*.

| | Fix |
|---|---|
| **Stick → cursor** | The D-pad (afternoon) took one discrete step per push, so on a ring of nine it went one wedge and stopped. `stick-nav.ts` rewritten: `moveCursor` advances a scene point at `cursorSpeed(m)` = 150 units/s × linear deflection (stickVector's `m` is squared, so `sqrt`), held inside the rim (slides round it; the hub is open, so it crosses to the far side); `wedgeUnder` names the wedge under it at any ring (hub = nothing; nowhere keeps the previous — no flicker); `followPoint` scrolls the camera only when the point leaves the inner 22 % box. `FlavorWheel`: the loop integrates it while the knob is held, `setFocus` + `hapticTick` on every wedge change, a `.wheel-cursor` dot positioned per frame (HTML overlay, `left/top`, like the press ring), `selectAtCursor` on release — a wedge of an UNFRAMED family frames the family (not `flyTo(leaf)` at 3×) and keeps the wedge highlighted; inside the framed family it toggles. Tap-on-glass-while-held selects the same way and disarms the release; a stick still pushed afterwards re-arms it (release always selects a hold that moved — test asserts this). The cursor restarts each hold on the highlighted wedge's centroid, else the visible centre. |
| **Tray** | `DescribeOverlay`: `onPointerDownCapture` on the stage collapses the tray on compact when the touch is outside it; the tray wrapper is `z-[8]` (above back 6 / home 5 / counter 6 / stick 7); the stick toggle moved to top 48 / right 12, out of the tray's band (it sat inside the collapsed row and over the open tray). |
| **Haptics** | `haptics.ts`: click the `<input type=checkbox switch>` directly (no label), keep it rendered but invisible; if not inside a trusted event (`window.event?.isTrusted`) queue the tick and play it once on the next pointer/touch event (document capture listener, armed on first need). Android path unchanged. |

**Verification:** `npx vitest run src/components/cupping/cva src/lib/haptics.test.ts src/lib/cva src/hooks src/types` → 379 passed / 27 files; `npx tsc --noEmit` 0; `npx eslint` 0; `npm run build` compiled. Test-writing lessons: after a family is framed the cursor restarts on that family's centroid (not straight up), so assert "one of Floral's children", not a specific leaf; and a stick still pushed after a tap-select keeps moving, so the "release selects nothing more" test must return the knob to centre first.

**Interpretation to confirm with Daniel:** "circular motion" was read as the thumb's analog motion on the stick plus the cursor sliding round the rim. If he meant the stick's ANGLE should map to the wheel's angle (point at 2 o'clock → the 2 o'clock family), that is a different model — a rotary dial — and `stick-nav.ts` is where it would go.
