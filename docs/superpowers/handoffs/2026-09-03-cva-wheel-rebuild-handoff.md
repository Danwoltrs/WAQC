...
**Resume point:** Feedback round 1 is **LIVE on production** (`9fbd177..0a23fea`). **Round 2 (`ed3b7fe`, no dimming) is committed on `main` but NOT pushed** — Daniel controls when, since a push deploys. Next, in order: (0) push `ed3b7fe` when he says so; (1) **physical parity QA** on a real desktop first (the three revisions below are desktop-felt: hover-dwell fly, labels growing with the zoom, the wheel lifting clear of the tray), then iPad/phone — checklist is Task 12 Step 3 of [../plans/2026-09-02-cva-wheel-rebuild.md](../plans/2026-09-02-cva-wheel-rebuild.md); (2) **sub-project 2** — the wheel on mainstream (commodity) qualities — locked decisions in the spec's appendix, **no spec yet**, starts with a brainstorm.

## Follow-up round 2 — 2026-09-03 (`ed3b7fe`, committed, NOT pushed)

Daniel on the round-1 build: *"a lot smoother now, but no need to hide the other sides, let them visible."* Framing a family dimmed the other eight to a muted grey at 0.42 opacity and switched their labels off; he wants the whole wheel readable while one family is framed.

**Removed outright, not softened.** `WheelScene` no longer emits `is-muted` or `--wheel-muted`; `palette.ts` lost `mutedColor`, `SURFACE` and `PaletteEntry.muted`; `globals.css` lost the two dimming rules, the `.is-muted .is-picked` override that existed only to survive them, and the `.wheel-fam` opacity transition they needed. `visibleLabelKeys` lost its `focusFamily` argument — label visibility is geometry alone, so zooming only ever ADDS labels. **If Daniel ever wants a hint of dimming back, `git revert ed3b7fe` restores the whole mechanism coherently** — do not rebuild it by hand.

**Two consequences worth knowing.** (1) Rule 2 is now literally true: there is NO transition anywhere inside the svg, so the scene is completely static. (2) `WheelScene` no longer takes `focusFamily` at all — the svg's `data-focus` had no reader (every test and handler uses the ROOT div's), so a drill now reconciles **none** of the scene's ~600 elements, where it previously re-rendered all of them to repaint eight families grey. That is a real drilling win on top of round 1.

**What now identifies the framed family:** the camera being on it, the `← Family` breadcrumb, and `cursor: pointer` (leaves of the framed family only). There is no colour cue. This matters because `activate()` still picks inside the framed family and re-aims outside it — with hover-dwell on, resting on a neighbour re-aims within 240 ms, so the mismatch window is small, but a fast click on a neighbour's leaf flies instead of picking. Watch for this in QA.

**Verification:** `npx vitest run` → **132 files / 1556 tests** (was 132 / 1554); `npx tsc --noEmit` → 0 errors; `npx eslint src/components/cupping/cva/wheel/` → clean. In Chrome on `/embed/wheel-harness` with Green/Vegetative framed: all nine families render their true fills at opacity 1, zero `is-muted` elements, all 110 labels displayed across all nine families; a sweep of all 577 elements inside the svg finds no non-zero `transition-duration`, no filter and no CSS transform.

## Follow-up round 1 — 2026-09-03 (`a134988`, LIVE prod)
...
14. **Nothing dims, ever (Daniel, 2026-09-03, round 2)** — framing a family leaves all nine in full colour with their labels on. Do not reintroduce muting, a fade, or a "subtle" 0.9 opacity as a focus affordance. `WheelScene` must stay ignorant of the framed family so drilling costs no reconcile.
...
3. **No filters anywhere inside `.wheel-root`.** This is the single biggest win; do not reintroduce a blur "just for the tray". (Round 2 went further: no dimming of any kind — see 14.)
...
- [labels.ts:111](../../../src/components/cupping/cva/wheel/labels.ts#L111) `visibleLabelKeys` (geometry only, no family filter); [palette.ts:49](../../../src/components/cupping/cva/wheel/palette.ts#L49) `PALETTE` (fill + label, no muted); [WheelScene.tsx:80](../../../src/components/cupping/cva/wheel/WheelScene.tsx#L80) — takes no `focusFamily`.
