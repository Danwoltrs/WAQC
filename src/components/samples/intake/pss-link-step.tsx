...
import { buildPssPickerOption, pssOfficialRef } from '@/lib/pss-picker-option'
...
export function PssLinkStep({ formData, approvedPSSSamples, onSelectPss, onClearPss }: Props) {
  ...
  const options = approvedPSSSamples.map(buildPssPickerOption)
      ...
      <div className="rounded-2xl p-3 bg-[#556b2f]/10 border border-[#556b2f]/30 min-w-[300px]">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold">Linked PSS #{pssOfficialRef(selected) || selected.tracking_number}</div>
