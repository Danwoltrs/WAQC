...
export const getUserPermissions = (role: UserRole, laboratoryType?: string): string[] => {
  const basePermissions: Record<UserRole, string[]> = {
    ...
    global_quality_admin: ['view_samples', 'create_samples', 'conduct_assessments', 'view_global_quality', 'view_all_labs', 'manage_global_quality_specs'],
  ...
  if (laboratoryType === 'hq' && role === 'lab_finance_manager') {
...
export const canAccessLab = (userRole: UserRole, userLabId: string | undefined, targetLabId: string): boolean => {
  // Global roles can access all labs
  if (['santos_hq_finance', 'global_finance_admin', 'global_quality_admin', 'global_admin'].includes(userRole)) {
    return true
  }
...
export const canAccessFinanceData = (userRole: UserRole, userLabId: string | undefined, targetLabId: string): boolean => {
  // Santos HQ finance can access all labs
  if (userRole === 'santos_hq_finance' || userRole === 'global_finance_admin' || userRole === 'global_admin') {
    return true
  }

  // Lab finance managers can only access their own lab's data
  if (userRole === 'lab_finance_manager' && userLabId === targetLabId) {
    return true
  }
