/**
 * POST /api/reports/weekly-ss/send
 *
 * Generates the Weekly SS Certificates PDF and emails it via Microsoft Graph
 ...
 *   subject (string, optional — defaults to "<Client> Weekly SS Certificates · <date range>")
...
export async function POST(request: NextRequest) {
  try {
    ...
    const subject = (typeof subjectIn === 'string' && subjectIn.trim().length > 0)
      ? subjectIn.trim()
      : `${report.data.client.name} · Weekly SS Certificates · ${periodLabel}`
    ...
    console.error('Error in POST /api/reports/weekly-ss/send:', error)
