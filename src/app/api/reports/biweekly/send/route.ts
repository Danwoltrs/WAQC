/**
 * POST /api/reports/biweekly/send
 *
 * Generates the Bi-Weekly Performance PDF and emails it via Microsoft Graph
 ...
 *   start_date / end_date (ISO date, required)
 *   to (string[], required, ≥1 valid address)
 *   cc, bcc (string[], optional)
 *   subject (string, optional — defaults to "<Client> · Bi-Weekly Performance · <date range>")
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Default subject / body. Plain-text only for v1; HTML composer is a
    // polish iteration once the basic send works.
    ...
    const bodyText = (typeof bodyIn === 'string' && bodyIn.trim().length > 0)
      ? bodyIn
      : `Hello,\n\nPlease find attached the Bi-Weekly Performance report for ${report.data.client.name} covering ${periodLabel}.\n\nBest regards,\n${senderName ?? 'Quality Control'}\nWolthers & Associates`
    ...
    console.error('Error in POST /api/reports/biweekly/send:', error)
